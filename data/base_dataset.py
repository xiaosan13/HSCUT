...
def get_mask_transform(opt, params=None, method=Image.NEAREST):
    """Get transformation specifically for mask images.
    
    Masks are binary images that require:
    1. Same geometric transformations as domain A images
    2. No color-related processing
    3. Nearest neighbor interpolation to avoid blurring edges
    
    Parameters:
        opt -- options
        params -- transformation parameters (crop position, flip, etc.)
        method -- interpolation method (default: NEAREST for binary masks)
    """
    transform_list = []
    
    # Geometric transformations (same as in get_transform)
    if 'fixsize' in opt.preprocess:
        transform_list.append(transforms.Resize(params["size"], method))
    if 'resize' in opt.preprocess:
        osize = [opt.load_size, opt.load_size]
    ...
    # For masks, only convert to tensor, no normalization
    # Masks are binary images (0-255), ToTensor will automatically convert to 0-1 range
